...
public class ClientDaoImpl implements ClientDao {
    ...
    public Client findClientById(int id) {
        ...
        try (PreparedStatement preparedStatement = connection.prepareStatement(FIND_CLIENT_SQL)) {
            ...
            try (ResultSet rs = preparedStatement.executeQuery()) {
                while(rs.next()){
                    cl = new Client(rs.getInt(1),rs.getString(2),rs.getString(3),rs.getString(4),rs.getString(5),rs.getInt(6),rs.getString(7),rs.getString(8));
                }
            }
    ...
    public Client finClientByUsernameAndPassword(String username, String password) {
        ...
        try (PreparedStatement preparedStatement = connection.prepareStatement(FIND_CLIENT_EXIST_SQL)) {
            ...
            try (ResultSet rs = preparedStatement.executeQuery()) {
                while(rs.next()){
                    c = new Client(rs.getInt(1),rs.getString(2),rs.getString(3),rs.getString(4),rs.getString(5),rs.getInt(6),rs.getString(7),rs.getString(8));
                }
            }
        } catch (SQLException e) {
            e.printStackTrace();
